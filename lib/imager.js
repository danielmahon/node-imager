...
Imager.prototype = {
  ...
  prepareUpload: function (file, filename, variant, fn) {
    ...
    if (variants.original) {
      var uploadFiles = function(cb) {
        self.uploadOriginal(file, filename, cb);
      };
      asyncArr.push(uploadFiles);
    }
    ...
    if (variants.resizeAndFill) {
      Object.keys(variants.resizeAndFill).forEach(function (name) {
      	if (name !== 'options') {
	        var processFiles = function (cb) {
	          var preset = {
	            name: name,
	            size: variants.resizeAndFill[name],
	            options: variants.resizeAndFill.options
	          };
	          self.resizeAndFill(file, preset, filename, cb);
	        };
	        asyncArr.push(processFiles);
      	}
      });
  ...
  uploadOriginal: function(file, fname, cb) {
    var filename = fname + contentType[file['type']];
    var self = this;
    var remoteFile = 'original_' + filename;
    self['pushTo' + self.storage](file['path'], remoteFile, filename, file['type'], cb);
  },
  ...
  resizeAndFill: function (file, preset, fname, cb) {
    var self = this;
    var ct = {
      'image/jpeg': '.jpg',
      'image/png': '.png',
    ...
    var fileType = preset.options.fileType || file['type'];
    var ext = ct[fileType];
    var filename = fname + ext;
    var remoteFile = preset.name + '_' + filename;
    var dest = tempDir + remoteFile;

    var output = gm(file['path']).autoOrient();
    
    if (preset.options.quality) {
    	output = output.quality(preset.options.quality);
    }
    if (preset.options.compress) {
    	output = output.compress(preset.options.compress);
    }
      
    output.resize(preset.size.split('x')[0], preset.size.split('x')[1])
      .gravity('Center')
      .background('black')
      .extent(preset.size.split('x')[0], preset.size.split('x')[1])
    
    output.write(dest, function (err) {
        if (err) return cb(err);
        self['pushTo' + self.storage](dest, remoteFile, filename, fileType, cb);
      });
  },
  ...
  prepareRemove: function (file, fn, variant) {
    ...
    if (variants.resizeAndCrop) {
      Object.keys(variants.resizeAndCrop).forEach(function (preset) {
		...
		if (variants.resizeAndFill) {
			Object.keys(variants.resizeAndFill).forEach(function(preset) {
				if (preset !== 'options') {
					var removeFiles = function(cb) {
						self['removeFrom' + self.storage](file, preset, cb);
					};
